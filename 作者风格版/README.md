...
脚本互相独立：`1` 用公开真实 ERP 数据，`2` 用带真值的仿真数据，`3` 验证四类 ERP/ERF，`4` 用 DBS 患者的 MEG 做 beta 频谱定位，`5` 专门修正 ds006035 左指运动定位，`6` 从 `5` 的结果快速渲染完整 pial 俯视图，`7` 批量运行当前 OASTER-ERP 的全头七方法多 SNR 仿真。两个 `9` 脚本分别补齐真实 ERP 和 DBS beta 的可比方法，`10` 补左指运动 beta，`11` 只读已有源图绘制六方法脑表面图。编号表示阅读顺序，不要求依次运行。
...
用途：在同一 EEG/MEG SNR 下，为纯表层、纯深层、深层加单表层、深层加双表层各画一个固定代表配置。纯表层与“深层+单表层”共享一个俯视可见的高位皮层点，三个含深层场景共享同一丘脑点，便于解释差异。每种情形都把仿真真值单独画出，不把真值标记叠在算法图上；含深层源时同时输出 MRI 解剖渲染和完整 dorsal 俯视 pial 皮层渲染。脚本最后检查每张图并生成 `INDEX.md`，其中明确记录 `case_id`、`configuration_number`、表层顶点和深层点。

## 真实数据多方法对照

`9-ds006035八方法ERP对比.py` 在相同 EEG/MEG evoked、前向模型和 N20/P30 时间窗上比较 OASTER-ERP-v3 与原定七种时域方法；先按 run 保存源图和指标，再按四名留出受试者各自三个 run 的中位数汇总。当前仅使用模板皮层源网格，真实数据无源真值，不能计算 AUC/DLE。

`9-DBS多方法beta对照.py` 复用 `4` 的同一批 rest/Hold epoch 和体积源网格，在独立 v3 目录比较 DICS、LCMV 与最小范数家族的 13–30 Hz beta-ERD。网格偶极拟合、RAP-MUSIC 和旧频谱 OASTER 尚不能直接估计同一个 beta 功率差，表中标为“需适配”，不填数值。

`10-左指运动多方法beta对照.py` 复用 `5` 的严格配对和三个 run，给 Finger beta-ERD 增加同频段的 LCMV 和最小范数家族。已在 sm09 的 113 个可用试次上完整跑通；只有一名受试者，不能将 run 当作独立样本。

`11-左指运动beta六方法脑图.py` 只读取 `10` 已保存的逐源 beta-ERD 图并渲染六方法 pial 俯视图，不重复约 10 分钟的逆解；各图各自峰值归一化/P95 显示，不能用颜色亮度比较绝对振幅。
