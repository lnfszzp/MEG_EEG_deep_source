# %%
# 开发诊断：同四个开发 case，检查空间边缘惩罚/源惩罚的相对强度。
...
save_root = project_root / "results/erp_whole_head/adaptive_v4/spatial_penalty_screen_01"
sample_data_path = Path(r"D:\mne_data\MNE-sample-data")
edge_fractions = (2.0, 5.0, 10.0)
...
all_rows = []
fieldnames = (*simulation.archive.ROW_FIELDS, "edge_fraction", "noise_multiplier")
started = time.perf_counter()


# %% 2. 顺序运行 6 个参数组合 × 4 个固定 case；每个参数组合独立保存。
for edge_fraction in edge_fractions:
    for noise_multiplier in noise_multipliers:
        assert simulation._code_fingerprint(fingerprint_paths) == code_fingerprint, "运行期间代码发生变化"
        tag = f"edge_{edge_fraction:g}_noise_{noise_multiplier:g}".replace(".", "p")
        ...
        runtime = {
            ...
            "oaster_kwargs": {"edge_fraction": edge_fraction, "noise_multiplier": noise_multiplier},
            "diagnostics_dir": diagnostics_root,
        }
        setting_rows = []
        for case in selected:
            row = simulation._score_case(case, runtime, manifest_sha256)["OASTER-ERP-v4"]
            row.update(edge_fraction=edge_fraction, noise_multiplier=noise_multiplier)
...
assert len(diagnostic_files) == 24
