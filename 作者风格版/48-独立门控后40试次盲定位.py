...
parser.add_argument("--development-threshold", type=float, default=.01)
...
lines = ["# 独立门控后40试次盲定位（开发闭环）", "",
    f"H0/H1 由 check20 的独立{summary['score_kind']}分数选择，决定后才合并40试次重定位；决定过程不读取真值。{development_threshold:g}是开发阈值，只用于闭环检查，正式流程必须换成纯表层校准规则。", "",
...
lines += ["", f"开发集盲选正确：{summary['family_correct_posthoc_count']}/{len(rows)}；纯表层误报：{summary['pure_surface_false_positive_count']}/{surface_count}；深源检出：{summary['deep_true_detected_count']}/{deep_count}。",
