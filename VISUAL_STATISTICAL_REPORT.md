...
> **源空间限制：** 本轮审计发现旧生成脚本遗漏 HEAD→MRI 变换，原计划的 15 个丘脑点实际位于脑干、小脑和第四脑室附近。以下 deep 指标只描述这批冻结非皮层点，不代表丘脑性能；详见 `SOURCE_SPACE_AUDIT.md`。
...
荧光绿色星形是仿真源精确中心，绿色空心圆是仿真源区域，magma 热图是估计源能量。每个方法按自身峰值归一化并仅显示不低于峰值 10% 的区域，所以这些图适合比较位置、扩散和伪源，不用于比较方法间绝对振幅。
